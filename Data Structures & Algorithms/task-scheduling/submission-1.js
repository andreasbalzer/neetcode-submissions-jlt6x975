...
class Solution {
    ...
    leastInterval(tasks, n) {
        const waitHeap = new Heap((item) => item[2]);
        ...
        let time = 0;
        while (readyHeap.size || waitHeap.size) {
            /*console.log("loop");
            console.log("readyHeap:");
            console.log(readyHeap.data);
            console.log("wait heap data:");
            console.log(waitHeap.data);*/
            while (waitHeap.size && waitHeap.data[1][2] <= time) {
                const readyTask = waitHeap.dequeue();
            ...
            if (readyHeap.size) {
                ...
                if (item[1]*-1 > 1) {
                    ...
                    waitHeap.enqueue([item[0], item[1]*-1 - 1, time + n + 1]);
                }
            }
            time += 1;
        }
