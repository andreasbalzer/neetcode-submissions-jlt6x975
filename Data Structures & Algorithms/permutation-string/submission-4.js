 ...
 */
        ...
        while (right < s2.length) {
            console.log(s1Map);
            console.log(`${s2} right ${right} s2 ${s2[right]}`);
            if (matches === 26) {
            ...
            if (s2Map.get(s2[left]) === (s1Map.get(s2[left]) || 0)) {
                ...
                console.log(`2 too many ${s2Map.get(s2[left])} ${s1Map.get(s2[left])}`)
                matches--;
            }   
            left++;
            right++;
            console.log(`matches at end ${matches}`);
            console.log(s2Map);
        }
